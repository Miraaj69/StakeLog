// BetsScreen.js — Phase 2: Timeline feed with date separators
...
// Date section header — timeline style
function DateHeader({ title, dayPnL, currSym, colors }) {
  var isPos = dayPnL >= 0;
  return (
    <View style={[dh.wrap, { backgroundColor: colors.background }]}>
...
var dh = StyleSheet.create({
  wrap: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, paddingHorizontal: 0 },
...
export default function BetsScreen() {
  ...
  var filtered = useMemo(function() {
    ...
    return bets.filter(function(b) {
      ...
      if (search) {
        ...
        if (!b.event.toLowerCase().includes(q) && !b.bet.toLowerCase().includes(q) &&
  ...
  // Group into sections by date for timeline
  var sections = useMemo(function() {
    ...
    return order.map(function(dk) {
      var dayBets = groups[dk];
      var dayPnL  = dayBets.reduce(function(s, b) {
        if (b.status === 'Won')  return s + parseFloat(b.stake) * (parseFloat(b.odds) - 1);
        if (b.status === 'Lost') return s - parseFloat(b.stake);
        return s;
      }, 0);
      return { title: dk, displayTitle: formatDateHeader(dk), dayPnL: dayPnL, data: dayBets };
    });
  }, [filtered]);
  ...
  var summaryItems = [
    { key: 'pnl',  label: 'P&L',     value: (stats.totalPnL>=0?'+':'')+formatMoney(stats.totalPnL,currSym), color: stats.totalPnL>=0?'#1A9E4A':'#D93025', bg: stats.totalPnL>=0?'#E8F8EE':'#FDECEA', border: stats.totalPnL>=0?'#A7DFB9':'#F5B8B2' },
    ...
    <SafeAreaView style={[s.screen, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <View style={[s.searchRow, { backgroundColor: colors.surfaceVariant, borderColor: search?'#E50914':colors.border }]}>
        ...
        <Pressable onPress={() => { setShowFilters(function(v) { return !v; }); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }}
          ...
          <Text style={{ fontSize: 12, color: showFilters?'#fff':colors.textSecondary, fontWeight: '700' }}>{showFilters?'▲':' ▼'} Filter</Text>
        </Pressable>
      </View>
      ...
      <SectionList
        ...
        renderSectionHeader={function(ref) {
          ...
          return <DateHeader title={section.displayTitle} dayPnL={section.dayPnL} currSym={currSym} colors={colors} />;
        }}
