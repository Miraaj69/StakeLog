...
function PinScreen({ mode, savedPin, onSuccess, onSetPin }) {
  ...
  return (
    <View style={[pinS.screen, { backgroundColor: colors.background }]}>
      <View style={[pinS.iconWrap, { backgroundColor: colors.primaryContainer, borderColor: colors.primaryBorder }]}>
        <Text style={{ fontSize: 36 }}>🔐</Text>
      </View>
      <Text style={[pinS.title, { color: colors.textPrimary }]}>
        {mode === 'set' ? 'Create PIN' : 'Enter PIN'}
      </Text>
      <Text style={[pinS.subtitle, { color: colors.textTertiary }]}>
      ...
      {err ? <Text style={[pinS.err, { color: colors.loss }]}>{err}</Text> : <View style={{ height: 20 }} />}
      <View style={pinS.keypad}>
        {keys.map((k, i) => (
          <Pressable key={i} onPress={() => press(k)} disabled={k === ''}
            ...
            <Text style={[pinS.keyText, { color: k === '⌫' ? colors.primary : colors.textPrimary }]}>{k}</Text>
          </Pressable>
        ))}
      </View>
    </View>
...
function OnboardingScreen({ onDone }) {
  ...
  return (
    <View style={[onbS.screen, { backgroundColor: colors.background }]}>
      <Animated.View entering={FadeIn.duration(300)} key={step} style={onbS.content}>
        <View style={[onbS.iconWrap, { backgroundColor: colors.primaryContainer, borderColor: colors.primaryBorder }]}>
      ...
      <View style={onbS.bottom}>
        <View style={onbS.dotsRow}>
          {steps.map((_, i) => (
            <View key={i} style={[onbS.dot, { backgroundColor: i === step ? colors.primary : colors.border, width: i === step ? 22 : 7 }]} />
          ))}
        </View>
        <Pressable onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); step < steps.length - 1 ? setStep(s => s + 1) : onDone(); }}
          style={[onbS.nextBtn, { backgroundColor: colors.primary }]}>
          <Text style={onbS.nextBtnText}>{step < steps.length - 1 ? 'Continue →' : "Let's Go 🚀"}</Text>
        </Pressable>
        {step > 0 && (
          <Pressable onPress={() => setStep(s => s - 1)} style={onbS.backBtn}>
            <Text style={[onbS.backText, { color: colors.textTertiary }]}>← Back</Text>
          </Pressable>
        )}
      </View>
    </View>
...
const onbS = StyleSheet.create({
  ...
  nextBtn: { borderRadius: 999, paddingVertical: 16, alignItems: 'center' },
  ...
  return (
    <View style={[tabS.bar, { backgroundColor: colors.surface, borderTopColor: colors.border }]}>
      {state.routes.map((route, index) => {
        ...
        const onPress = () => {
          scale.value = withSpring(0.85, { damping: 12 }, () => { scale.value = withSpring(1); });
          Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
        ...
        return (
          <Pressable key={route.name} onPress={onPress} style={tabS.tab}>
            <Animated.View style={[tabS.inner, animStyle]}>
              <View style={[tabS.iconWrap, isFocused && { backgroundColor: colors.primaryContainer }]}>
                <Text style={tabS.icon}>{TABS[index].icon}</Text>
              </View>
              <Text style={[tabS.label, { color: isFocused ? colors.primary : colors.textTertiary, fontWeight: isFocused ? '700' : '500' }]}>
                {TABS[index].label}
              </Text>
            </Animated.View>
          </Pressable>
...
function AppContent() {
  ...
  if (!onboarded) {
    return <OnboardingScreen onDone={async () => { await setItem(KEYS.ONBOARDED, true); setOnboarded(true); }} />;
  }
