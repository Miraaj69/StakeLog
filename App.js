...
function GlobalFAB({ currentTab }) {
  ...
  const handleAction = (id) => {
    if (id === 'add') {
      ...
      var tb = bets.filter(b => new Date(b.date).toDateString() === today);
      var pnl = tb.reduce(function(s, b) {
      ...
      Alert.alert(
        "Today's P&L",
        tb.length === 0 ? 'No bets today yet.' :
          'Bets: ' + tb.length + '\n' +
          'Won: ' + tb.filter(b => b.status === 'Won').length + '  Lost: ' + tb.filter(b => b.status === 'Lost').length + '\n' +
  ...
  return (
    <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, top: 0 }} pointerEvents="box-none">
      ...
      <AddBetModal
        ...
        editBet={null}
        bookies={useStore.getState().bookies}
        sports={useStore.getState().sports}
    ...
    <View style={{ flex: 1 }}>
      <Tab.Navigator
        tabBar={props => {
          ...
          React.useEffect(() => { setCurrentTab(name); }, [name]);
