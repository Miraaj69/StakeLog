        ...
        {keys.map((k, i) => (
          <Pressable key={i} onPress={() => press(k)} disabled={k === ''}
            style={({ pressed }) => [
              pinStyles.key,
              {
                backgroundColor: k === '' ? 'transparent' : pressed ? colors.primary : colors.surface,
                borderColor: k === '' ? 'transparent' : colors.border,
                opacity: k === '' ? 0 : 1,
              },
              ...Object.entries(Shadows.sm).map(([key, val]) => ({ [key]: val })),
            ]}>
  ...
  return (
    <View style={[onbStyles.screen, { backgroundColor: colors.background }]}>
      ...
      <View style={onbStyles.bottom}>
        ...
        <Pressable
          onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); step < steps.length - 1 ? setStep(s => s + 1) : onDone(); }}
  ...
  return (
    <View style={[
      ...
      {state.routes.map((route, index) => {
        ...
        const onPress = () => {
          scale.value = withSpring(0.85, { damping: 12 }, () => { scale.value = withSpring(1, { damping: 14 }); });
          Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
