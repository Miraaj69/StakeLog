// App.js — Premium v2: Indigo tab bar, updated colors
...
function PinScreen({ mode, savedPin, onSuccess, onSetPin }) {
  ...
  return (
    <View style={[pinS.screen, { backgroundColor: colors.background }]}>
      <View style={[pinS.iconWrap, { backgroundColor: 'rgba(124,107,255,0.12)' }]}>
      ...
      <Animated.View style={[pinS.dotsRow, shakeStyle]}>
        {[0,1,2,3].map(i => (
          <View key={i} style={[pinS.dot, { borderColor: digits.length > i ? '#7C6BFF' : colors.border }, digits.length > i && { backgroundColor: '#7C6BFF' }]} />
        ))}
      </Animated.View>
      ...
      <View style={pinS.keypad}>
        {keys.map((k, i) => (
          <Pressable key={i} onPress={() => press(k)} disabled={k === ''}
            style={({ pressed }) => [pinS.key, { backgroundColor: k==='' ? 'transparent' : pressed ? 'rgba(124,107,255,0.15)' : colors.surface, borderColor: colors.border, borderWidth: k==='' ? 0 : 0.5, opacity: k==='' ? 0 : 1 }]}>
            <Text style={[pinS.keyTxt, { color: k==='⌫' ? '#7C6BFF' : colors.textPrimary }]}>{k}</Text>
          </Pressable>
        ))}
      </View>
    </View>
...
function OnboardingScreen({ onDone }) {
  ...
  const steps = [
    ...
    { icon:'📊', title:'Smart Analytics', desc:'P&L charts, win rate, streak tracking, and AI-powered insights from your data.' },
    { icon:'🏆', title:'Gamification & Goals', desc:'Earn XP, unlock badges, set monthly goals, and track your progress.' },
  ...
  return (
    <View style={[onbS.screen, { backgroundColor: colors.background }]}>
      <Animated.View entering={FadeIn.duration(280)} key={step} style={onbS.content}>
        <View style={[onbS.iconWrap, { backgroundColor:'rgba(124,107,255,0.12)' }]}>
      ...
      <View style={onbS.bottom}>
        <View style={onbS.dotsRow}>
          {steps.map((_,i) => <View key={i} style={[onbS.dot, { backgroundColor: i===step ? '#7C6BFF' : colors.border, width: i===step ? 22 : 7 }]} />)}
        </View>
        <Pressable onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); step < steps.length-1 ? setStep(s=>s+1) : onDone(); }}
          style={[onbS.nextBtn, { backgroundColor: '#7C6BFF' }]}>
...
const onbS = StyleSheet.create({
  ...
  icon: { fontSize:52 },
  title: { fontSize:26, fontWeight:'800', textAlign:'center', marginBottom:14, letterSpacing:-0.5, lineHeight:32 },
  desc: { fontSize:16, textAlign:'center', lineHeight:26 },
  bottom: { gap:14 },
  dotsRow: { flexDirection:'row', gap:7, justifyContent:'center', marginBottom:4 },
  dot: { height:7, borderRadius:4 },
  nextBtn: { borderRadius:999, paddingVertical:16, alignItems:'center', shadowColor:'#7C6BFF', shadowOffset:{width:0,height:4}, shadowOpacity:0.3, shadowRadius:10, elevation:5 },
...
function CustomTabBar({ state, navigation }) {
  ...
  return (
    <View style={[tabS.bar, {
      backgroundColor: isDark ? 'rgba(10,10,15,0.97)' : colors.surface,
        ...
        return (
          <Pressable key={route.name} onPress={onPress} style={tabS.tab}>
            <Animated.View style={[tabS.inner, animStyle]}>
              <Text style={[tabS.emoji, { opacity: focused ? 1 : 0.5 }]}>{TABS[idx].emoji}</Text>
              <Text style={[tabS.label, {
                color: focused ? '#7C6BFF' : colors.textTertiary,
...
const tabS = StyleSheet.create({
  ...
  indicator: { width:18, height:3, borderRadius:2, backgroundColor:'#7C6BFF', marginTop:1 },
});
...
function GlobalFAB({ currentTab }) {
  ...
  const handleAction = (id) => {
    if (id === 'add') { setModalVisible(true); }
    else if (id === 'won' || id === 'lost') {
      const status = id === 'won' ? 'Won' : 'Lost';
      if (pendingBets.length === 0) { Alert.alert('No Pending Bets', 'Add a pending bet first.'); }
      else if (pendingBets.length === 1) { markStatus(pendingBets[0].id, status); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); }
      else {
        Alert.alert('Mark as ' + status, 'Which bet?', [
          ...pendingBets.slice(0, 6).map(b => ({ text: String(b.event || 'Bet').substring(0, 35), onPress: () => { markStatus(b.id, status); Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); } })),
          { text: 'Cancel', style: 'cancel' },
        ]);
      }
    } else if (id === 'stats') {
      var todayCount = bets.filter(function(b) { return new Date(b.date).toDateString() === new Date().toDateString(); });
      Alert.alert("Today's P&L", todayCount.length === 0 ? 'No bets today yet.' :
        'Bets: ' + todayCount.length + '\nWon: ' + todayCount.filter(b => b.status === 'Won').length +
        '  Lost: ' + todayCount.filter(b => b.status === 'Lost').length + '\nP&L: ' + (stats.todayPnL >= 0 ? '+' : '') + '₹' + Math.abs(stats.todayPnL).toFixed(0),
        [{ text: 'OK' }]);
    }
  };
  ...
  return (
    <View style={{ position:'absolute', bottom:0, left:0, right:0, top:0 }} pointerEvents="box-none">
      <FloatingMenu onAction={handleAction} hasPendingBets={pendingBets.length > 0} isProfit={stats.totalPnL >= 0} totalPnL={stats.totalPnL} />
      <AddBetModal visible={modalVisible} onClose={() => setModalVisible(false)}
        onSave={async (form) => { await addBet(form); setModalVisible(false); }}
        editBet={null} bookies={bookies} sports={sports} templates={templates}
        suggestStake={stats.suggestedStake} currSym="₹" />
    </View>
  );
}
...
function TabBarWithFAB({ state, navigation, descriptors, setCurrentTab }) {
  ...
  React.useEffect(() => { setCurrentTab(currentName); }, [currentName]);
...
function MainTabs() {
  ...
  return (
    <View style={{ flex:1 }}>
      <Tab.Navigator tabBar={props => <TabBarWithFAB {...props} setCurrentTab={setCurrentTab} />} screenOptions={{ headerShown:false }}>
...
function AppContent() {
  ...
  useEffect(() => {
    migrateIfNeeded();
    init();
    Promise.all([getItem(KEYS.ONBOARDED, false), getItem(KEYS.PIN_ENABLED, false), getItem(KEYS.PIN, '')]).then(([ob, pe, p]) => {
      setOnboarded(ob); setPinEnabled(pe); setSavedPin(p);
    });
  }, []);
