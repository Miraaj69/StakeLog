...
import {
  ...
  calcROI, DEFAULT_BOOKIES, DEFAULT_SPORTS, makeForm,
...
export function useBets() {
  ...
  const saveBookies = useCallback(async (newBookies) => {
    setBookies(newBookies);
    await setItem(KEYS.BOOKIES, newBookies);
  }, []);

  const saveSports = useCallback(async (newSports) => {
    setSports(newSports);
    await setItem(KEYS.SPORTS, newSports);
  }, []);

  const saveBankroll = useCallback(async (amount) => {
    setBankrollStart(amount);
  ...
  return {
    bets, bookies, sports, bankrollStart, templates, loading,
    undoStack, stats,
